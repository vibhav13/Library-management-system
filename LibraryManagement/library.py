...
class menu:
    ...
    def adddata(self):
        ...
        try:
            ...
            messagebox.showinfo("Error","Enter correct value of copies.")
        f=self.aloc.get()
        conn=sqlite3.connect('test.db')
        try:
    ...
    def issuedbook(self):
        ...
        if (bookid and studentid!=""):
            r = conn.execute("select count(?) from book_issued where STUDENT_ID = ?",(studentid.capitalize(),studentid.capitalize(),))
            for i in r.fetchall():
                if i[0]>=2:
                    messagebox.showinfo("Error","Already 2 books issued by the student.")
                elif an!=[]:
                    for i in an:
                        if i[1]>0:
                            try:
